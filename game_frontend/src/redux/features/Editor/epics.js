...
const getCodeEpic = (action$, store$, { api }) =>
  action$.pipe(
    ...
    mergeMap(action =>
      api.get(`code/${store$.value.game.connectionParameters.id}/`).pipe(
...
const postCodeEpic = (action$, store$, { api }) =>
  action$.pipe(
    ...
    api.post(
      `/aimmo/api/code/${store$.value.game.connectionParameters.id}/`,
...
const changeCodeEpic = (action$, scheduler = backgroundScheduler) =>
  action$.pipe(
